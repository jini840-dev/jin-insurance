...
import { getFirestore, collection, addDoc, getDocs, query, where, updateDoc, doc } from "https://www.gstatic.com/firebasejs/10.11.1/firebase-firestore.js";
...
async function loginUser(phone, nickname) {
    ...
    if (!db) {
        alert("데이터베이스가 초기화되지 않았습니다. 인터넷 연결을 확인하세요.");
        return;
    }
    ...
    try {
        ...
        alert(`로그인 오류: ${error.message}\n(Firestore Database가 활성화되어 있는지 확인하세요)`);
    }
}

// --- 이벤트 리스너 설정 (DOM 준비 후 실행) ---
function init() {
    ...
    // 모달 닫기
    const btnModalClose = getEl('btn-modal-close');
    if (btnModalClose) {
    ...
    // 실제 로그인 버튼 (시작하기)
    const btnPhoneSubmit = getEl('btn-phone-submit');
    if (btnPhoneSubmit) {
        btnPhoneSubmit.onclick = async () => {
    ...
    if (inputPhone) {
        ...
        // 포커스가 벗어날 때(onblur) 닉네임 조회
        inputPhone.onblur = async () => {
            const phoneVal = inputPhone.value.trim();
            if (phoneVal.length >= 12) { // 010-000-0000 형식 이상일 때
                console.log("Checking nickname for:", phoneVal);
                try {
                    const usersRef = collection(db, "users");
                    const q = query(usersRef, where("phone", "==", phoneVal));
                    const querySnapshot = await getDocs(q);
                    ...
                    if (!querySnapshot.empty) {
                        ...
                        if (inputNickname) {
                            ...
                            console.log("Nickname found and filled:", userData.nickname);
                        }
                    }
                } catch (error) {
                    console.error("Auto-fetch nickname error:", error);
                }
            }
        };
    }
    ...
    // 퀴즈 시작
    const btnStartQuiz = getEl('btn-start-quiz');
    if (btnStartQuiz) {
    ...
    const btnAdminAuthSubmit = getEl('btn-admin-auth-submit');
    if (btnAdminAuthSubmit) {
        btnAdminAuthSubmit.onclick = () => {
...
// 퀴즈 관련 함수들 (전역 스코프 유지 필요 시 함수 선언식 사용)
